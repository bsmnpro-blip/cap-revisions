(() => {
  ...
  function submitDialog(form) {
    ...
    if (kind === 'subject') { const item = R.createSubject({ ...values, id }); error = R.validateSubject(item); if (!error) { const index = state.data.subjects.findIndex((subject) => subject.id === id); if (index >= 0) state.data.subjects[index] = { ...state.data.subjects[index], ...item }; else state.data.subjects.push(item); } }
    if (kind === 'course') { const item = R.createCourse({ ...values, id }); error = R.validateCourse(item, state.data.subjects); if (!error) { const index = state.data.courses.findIndex((course) => course.id === id); if (index >= 0) state.data.courses[index] = { ...state.data.courses[index], ...item }; else state.data.courses.push(item); } }
